...
def project_op(model, proj_queue, args, infer, cell_type, selected_eid=None):
    ...
    for opid in range(num_ops):
        ...
        valid_stats = infer(proj_queue, model, log=False, _eval=False, weights_dict=weights_dict, pt=False)
...
def pt_project(train_queue, valid_queue, model, architect, optimizer,
    ...
    for epoch in range(start_epoch, tune_epochs):
        ...
        for step, (input, target) in enumerate(train_queue):
            ...
            architect.step(input, target, input_search, target_search, mask_search, pt=False, return_logits=True)
            ...
            logits, loss = model.step(input, target, args, mask, pt=False)
        ...
        train_acc, train_obj = infer(train_queue, model, log=False, pt=False)
        logging.info('train_acc  %f', train_acc)
        logging.info('train_loss %f', train_obj)

        valid_acc, valid_obj = infer(valid_queue, model, log=False, pt=False)
        logging.info('valid_acc  %f', valid_acc)
        logging.info('valid_loss %f', valid_obj)
