...
import os
import sys
sys.path.insert(0, '/data/gbc/Workspace/darts-pt/')
import time
import glob
import random
import numpy as np
...
expid = args.save
args.save = '/data/gbc/Workspace/darts-pt/results/{}/search-{}-{}-seed{}'.format(
...
if args.resume_epoch > 0: # do not delete dir when resume:
    args.save = '/data/gbc/Workspace/darts-pt/results/{}/{}'.format(args.dataset, args.resume_expid)
    assert(os.path.exists(args.save)), 'resume but {} does not exist!'.format(args.save)
else:
    scripts_to_save = glob.glob('/data/gbc/Workspace/darts-pt/sota/cnn/*.py') + glob.glob('/data/gbc/Workspace/darts-pt/nasbench201/architect*.py') + glob.glob('/data/gbc/Workspace/darts-pt/optimizers/darts/architect.py')
